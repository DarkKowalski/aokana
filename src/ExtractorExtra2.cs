...
namespace Aokana
{
    public class ExtractorExtra2 : IExtractor
    {
        ...
        public int ExtractAll(string dataFilePath, string toDirectory)
        {
            ...
            fs.Position = 0L;
            Dictionary<string, ExtractorExtra2.MapKey> map = new Dictionary<string, ExtractorExtra2.MapKey>();

            byte[] buffer = new byte[1024];
            fs.Read(buffer, 0, 1024);

            int numFiles = 0;
            for (int index = 3; index < (int)byte.MaxValue; ++index)
            ...
            for (int index2 = 0; index2 < numFiles; ++index2)
            {
                ...
                while (index3 < rpaths.Length && rpaths[index3] != (byte)0)
                    ++index3;
                map.Add(Encoding.ASCII.GetString(rpaths, index1, index3 - index1).ToLower(), new ExtractorExtra2.MapKey()
                {
                    p = uint32_3,
                    L = uint32_1,
                    k = uint32_2
            ...
            foreach (KeyValuePair<string, ExtractorExtra2.MapKey> keyValuePair in map)
            {
                String fileName = keyValuePair.Key;
                ExtractorExtra2.MapKey mapKey;
        ...
        {
            uint num1 = (uint)((int)k0 * 4892 + 42816);
            uint num2 = num1 << 7 ^ num1;
            for (int index = 0; index < 256; ++index)
            {
                uint num3 = num1 - k0 + num2;
                num2 = num3 + 156U;
                uint num4 = num3 * (num2 & 206U);
                b[index] = (byte)num4;
                num1 = num4 >> 3;
            }
        }
        ...
        private void Pass1(byte[] b, int L, uint k)
        {
            ...
            {
                byte num = (byte)((uint)(byte)((uint)(byte)((uint)(byte)((uint)b[index] ^ (uint)b1[index % 179]) + 3U) + (uint)b1[index % 89]) ^ 119U);
                b[index] = num;
            }
        }
    }
